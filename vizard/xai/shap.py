...
class FlamlTreeExplainer:
    ...
    def __validate_sample_shape(sample: np.ndarray) -> np.ndarray:
        ...
        if sample.ndim != 2:  # sample(s) must be 2D
            raise ValueError(f'Given sample is "{sample.ndim}", which must be 2D.')
    ...
    def overall_score(self, sample: np.ndarray) -> float:
        ...
        if shap_output.values.ndim == 3:
            score: np.ndarray = (
                np.sum(shap_output.values[:, :, 0]) - shap_output.base_values[:, 0]
            )
            return score.item()
    ...
    def top_k_score(self, sample: np.ndarray, k: int = 1) -> dict:
        ...
        top_k_feature_names: np.ndarray = np.array(self.feature_names)[top_k_idx]
    ...
    def aggregate_shap_values(
        ...
        for _category, _features in feature_category_to_feature_name.items():
            ...
            features_idx = self._get_indices(
                sublist=_features, superlist=self.feature_names
            )
            ...
            if shap_output.values.ndim == 3:
                aggregated_shap_values[_category] = np.sum(
                    shap_output.values[:, :, 0][:, features_idx]
                )
            else:
                aggregated_shap_values[_category] = np.sum(
                    shap_output.values[:, features_idx]
                )
