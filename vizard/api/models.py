...
class Payload(BaseModel):
    ...
    def _country_where_applying_country(cls, value):
        if value.upper() not in CountryWhereApplying.get_member_names():
            raise ValueError(f'Country "{value}" is not valid in this system.')
        return value
    ...
    @field_validator("contact_relation_to_me", "contact_relation_to_me2")
    ...
    @field_validator("occupation_title1", "occupation_title2", "occupation_title3")
    ...
    country_where_applying_period: float = 30.0  # days
    ...
    previous_marriage_period: float = 0.0  # years

    @field_validator("marriage_period", "previous_marriage_period")
    def _marriage_period(cls, value):
        if value < 0:
            raise ValueError("Value cannot be negative")
        return value

    passport_expiry_date_remaining: float = 3.0  # years
    ...
    how_long_stay_period: float = 30.0  # days
    ...
    education_period: float = 0.0  # years
    ...
    occupation_period2: float = 0.0  # years
    occupation_period3: float = 0.0  # years

    @field_validator("occupation_period", "occupation_period2", "occupation_period3")
    ...
    def _child_mother_father_spouse_foreigner_count(cls, value):
        if value < 0:
            raise ValueError("Value cannot be negative.")
        total_count: int = (
            CanadaGeneralConstants.MAXIMUM_CHILD_COUNT
    ...
    child_average_age: float = 0.0  # years
    ...
    def __init__(self, **data):
        ...
        if "occupation_title2" in data:
            data["occupation_title2"] = __occupation_title_x(
                value=data["occupation_title2"]
            )
        if "occupation_title3" in data:
            data["occupation_title3"] = __occupation_title_x(
                value=data["occupation_title3"]
            )
        ...
        if "applicant_marital_status" in data:
            data["applicant_marital_status"] = __marital_status(
        ...
        if "country_where_applying_status" in data:
            data["country_where_applying_status"] = __residence_status(
                data["country_where_applying_status"]
            )
