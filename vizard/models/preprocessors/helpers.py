...
def preview_column_transformer(
    ...
    df: pd.DataFrame,
    random_state: int | np.random.Generator = np.random.default_rng(),
    **kwargs,
) -> pd.DataFrame:
    ...
    Args:
        ...
        random_state (int | np.random.Generator, optional): A seed value or
    ...
    sample_indices = random_state.choice(
        original.shape[0], size=n_samples, replace=False
    )
    ...
    for idx, k in enumerate(ct.output_indices_):
        ...
        transformed_columns_slice: list | slice = ct.output_indices_[k]
        if isinstance(transformed_columns_slice, slice):
            transformed_columns_range = range(transformed_columns_slice.stop)
            transformed_columns_range = transformed_columns_range[
                transformed_columns_slice
            ]
        ...
        if not isinstance(ct.transformers[idx][1], OneHotEncoder):
            ...
            transformed_sample = transformed[
                sample_indices, transformed_columns_indices
            ]
            # fix shapes to be 2d
            original_sample = original_sample.reshape(
                sample_indices.shape[0], columns_indices.__len__()
            )
        ...
        # show info about one hot encoder changes
