...
feature_names = [
    "sex",
    "education_field_of_study" "occupation_title1",
...
print(len(feature_names), len(powerset(feature_names)))


def sample_maker(
    feature_names: List, feature_values: Dict[str, List]
) -> List[Dict[str, Any]]:
    ...
    """
    powerset_list = powerset(feature_names)
    samples = []
    while powerset_list:
        z = powerset_list.pop()
        sub_dict = sub_dict_with_keys(
            z, feature_values
        )  # dict with only keys that are in our sub list
        samples.extend(product_generator(sub_dict))
    return samples
