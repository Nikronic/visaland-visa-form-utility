...
class FLAMLDTreeViz:
    ...
    @staticmethod
    def __plot(plot: Optional[Path] = None, name: Optional[str] = None) -> None:
        ...
        if isinstance(plot, Path):
            if name is None:
                raise ValueError('`name` has to be provided when `plot` is not none')
            plt.savefig(plot / name)
        else:
            plt.show()

    def viz_leaf_samples(self, tree_index: int,
    ...
    def dtreeviz(self, tree_index: int,
                 scale: float = 1.0,
                 orientation: str = 'TD',
                 plot: Optional[Path] = None,
                 *args, **kwargs) -> None:
    ...
    def explain_prediction_path(self, tree_index: int,
                                instance_index: int,
                                plot: Optional[Path] = None,
                                explanation_type: str = 'plain_english',
        ...
        if explanation_type == 'plain_english':
            ...
            raise ValueError(f'`explanation_type` "{explanation_type}" is not valid')
