...
def fillna_datetime(dataframe: pd.DataFrame, col_base_name: str) -> None:
    """
...
def aggregate_datetime(dataframe: pd.DataFrame, col_base_name: str, new_col_name: str,
    ...
    """
    In a Pandas Dataframe, takes two columns of dates in string form and calculates the period of these two dates
        and represent it in integer form. The two columns used will be dropped.
    If no date is provided for given `col_base_name` that is empty,
        `fill_type` must be provided and it uses `reference_date` and `current_date` to
        fill one side of the dates (e.g. on going marriage).
