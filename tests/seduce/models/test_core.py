import numpy as np
from vizard.seduce.models.core import SigmoidSeducer
...
class TestSigmoidSeducer:
    def test_sigmoid(self):
        output_float = zad.sigmoid(input_float, scaler1)
        correct_float = 0.9608342772032357
        assert output_float == correct_float, "float sigmoid failed"
        output_numpy: np.ndarray = zad.sigmoid(input_numpy, scaler1)
    ...
    def test_adjusted_sigmoid(self):
        output_float = zad.adjusted_sigmoid(
        ...
        output_numpy: np.ndarray = zad.adjusted_sigmoid(
            input_float, adjusted_min, adjusted_max, scaler1
        )
        correct_numpy = np.array([0.01, 0.2575, 0.50506645, 0.7525, 1.0])
        assert (
            output_numpy.all() == correct_numpy.all()
        ), "np array adjusted sigmoid failed"

    def test_bi_level_adjusted_sigmoid(self):
        output_float = zad.bi_level_adjusted_sigmoid(
            input_float,
            adjusted_min,
            adjusted_max,
            closer_adjusted_min,
        ...
        correct_float = 0.39975041607715656
        assert output_float == correct_float, "float bi level adjusted sigmoid failed"
        output_numpy: np.ndarray = zad.bi_level_adjusted_sigmoid(
            input_numpy,
            adjusted_min,
            adjusted_max,
            closer_adjusted_min,
